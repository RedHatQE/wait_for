...
def wait_for(func, func_args=[], func_kwargs={}, logger=None, **kwargs):
    ...
    while t_delta <= num_sec:
        ...
        try:
            ...
            if _check_must_be_handled(e, handle_exception):
                ...
                exc = e
                logger.info(
                    "Call failed with following exception, but continuing "
        ...
        if out is fail_condition or fail_condition_check(out):
            ...
            if not quiet:
                logger.debug(
                    "Took %(time).2f to do %(message)r", {"time": duration, "message": message}
                )
    ...
    if filename and line_no:
        ...
        timeout_msg = f"Could not do '{message}' at {filename}:{line_no} in time"
    else:
        logger_fmt = "Couldn't complete %(message)r in time, took %(duration).2f, %(tries)d tries"
        logger_dict = {"message": message, "duration": t_delta, "tries": tries}
        timeout_msg = f"Could not do '{message}' in time"
